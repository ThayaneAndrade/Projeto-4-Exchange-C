    ...
    while (*indice_logado != -1) {
        ...
        switch (opcao) { //metodo switch para manipular as opções do menu
            ...
            case 3:
                ...
                break;
            case 9:
                arquivar_usuarios(Lista, *indice_logado); //atualiza os usuários no arquivo antes de sair
                printf("\n*Saindo...\n");
                *indice_logado = -1;
                break;
            default:
...
void carregar_usuarios(lista *Lista) { //lê usuários armazenados no arquivo e coloca no vetor de ponteiros.
    ...
    free(u); //libera o espaço alocado para o ultimo usuario, que não foi adicionado ao vetor
    fclose(arquivo); 
}

void arquivar_usuarios(lista *Lista, int indice_logado) { //função que salva os usuários no arquivo, caso o programa seja encerrado.
    FILE *arquivo = fopen("usuarios.bin", "wb");
    for (int i = 0; i < Lista->qtd; i++) { //percorre o vetor de ponteiros e salva cada usuario no arquivo
        fwrite(Lista->vetor[i], sizeof(usuario), 1, arquivo); 
    }
    fclose(arquivo);
}
 
void saldo(lista *Lista, int indice_logado) { //função que exibe o saldo do usuario logado.
...
void deposito(lista *Lista, int indice_logado) { //função que realiza o deposito na conta do usuario logado.
    if (solicita_senha(*Lista, indice_logado) == 0) {
        return;
    }
    float valor;
    ...
    Lista->vetor[indice_logado]->saldo += valor; //atualiza o saldo do usuario
